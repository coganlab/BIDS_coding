...
if __name__ == '__main__':
    ...
    files = ["/home/sbf/Desktop/git/BIDS_coding/BIDS_converter/testing/D52/D52"
             "_Session001_PhonemeSequencing_201213.ieeg.dat.gz",
                "/home/sbf/Desktop/share/workspace/D52_PhonemeSequencing_20121"
                "3.ieeg.dat"]
    for file in files:
        try:
            if file.endswith(".gz"):
                with gzip.open(file,'rb',config["compressLevel"]) as f:
                    # f_in=gzip.GzipFile(fileobj=f,mode='rb',compresslevel=con
                    # fig["compressLevel"])
                    # content = f.read()
                    ...
                    data = np.frombuffer(f.read(), dtype=np.dtype(
                        config["ieeg"]["binaryEncoding"]))
                    print(type(data))
            else:
                with open(file,mode='rb') as f:
                    data = np.fromfile(
                        f, dtype=config["ieeg"]["binaryEncoding"])
                    print(data.shape)
